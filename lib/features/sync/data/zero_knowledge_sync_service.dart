...
class ZeroKnowledgeSyncService {
  ...
  Future<SyncResult> sync({
    ...
    try {
      ...
      // 2. Pull: remote rows newer than sinceLastSync
      var query = _client
          .from(_table)
          .select()
          .eq('user_id', user.id)
      ...
      if (sinceLastSync != null) {
        query = query.gt('updated_at', sinceLastSync.toIso8601String());
      }
  ...
  Future<Uint8List> _getOrCreateSyncKey() async {
    ...
    await _storage.write(key: _syncKeyStorageKey, value: base64Encode(key));
    return key;
  }
  ...
  Future<VaultEntry?> _decryptRow(
    ...
    try {
      ...
      final plaintext =
          apiDecrypt(ciphertextWithNonce: ciphertext, key: syncKey);
      final json =
