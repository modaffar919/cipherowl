...
///   [32] AES-256-GCM nonce (12 bytes) + reserved (20 bytes)  ← actually [12] nonce
///   ...  see [_BackupEnvelope]
///
...
class BackupService {
  ...
  static Future<int> importFromFile(SmartVaultDatabase db) async {
    ...
    if (result == null || result.files.isEmpty) {
      throw const BackupException('لم يتم اختيار أي ملف');
    }

    final path = result.files.single.path;
    if (path == null) throw const BackupException('مسار الملف غير متاح');
  ...
  static Future<Uint8List> _loadKey() async {
    ...
    if (hex == null || hex.length != 64) {
      throw const BackupException('مفتاح قاعدة البيانات غير موجود');
    }
  ...
  static String _decryptEnvelope(Uint8List bytes, Uint8List key) {
    ...
    if (bytes.length < 5 + 12 + 16) {
      throw const BackupException('ملف النسخة الاحتياطية تالف أو غير مكتمل');
    }
    for (int i = 0; i < 4; i++) {
      if (bytes[i] != _kMagic[i]) {
        throw const BackupException('ملف غير صالح: ليس ملف CipherOwl Backup');
      }
    }
    ...
    try {
      ...
      throw const BackupException(
          'فشل فك التشفير — المفتاح خاطئ أو الملف تالف');
    }
  }
  ...
  static Future<int> _restoreFromJson(SmartVaultDatabase db, String json) async {
    ...
    try {
      ...
      throw const BackupException('بيانات النسخة الاحتياطية غير صالحة');
    }
